...
sebagai_survey() {
  ...
  runuser -u survey -- env PATH="$NODE22:/usr/bin:/bin" HOME=/home/survey bash -c "$1"
}
...
sebagai_survey "cd $REPO && npm run build"
sebagai_survey "cd $REPO && SIAPKAN_TERBIT=1 node scripts/prepare-application.mjs"
...
set +e
sebagai_survey "cd $APP && rm -rf $target && NEXT_DIST_DIR=$target npm run build"
hasil_build=$?
set -e
cp /tmp/terbit-next-env.d.ts "$APP/next-env.d.ts"
cp /tmp/terbit-tsconfig.json "$APP/tsconfig.json"
chown survey:survey "$APP/next-env.d.ts" "$APP/tsconfig.json"
...
if [ -d "$APP/$aktif/static" ]; then
  sebagai_survey "cp -rn $APP/$aktif/static/. $APP/$target/static/"
fi
...
chown survey:survey "$BERKAS_AKTIF"
...
sebagai_survey "rsync -a --delete $REPO/dist/assets/ $APP/public/assets/"
sebagai_survey "rsync -a $REPO/dist/_astro/ $APP/public/_astro/"
