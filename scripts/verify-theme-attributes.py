...
# Course cards are rendered in a loop over `src/data/courses.ts`, so how many
# times these appear follows the data, not the theme. The original page listed
# 22 Nod courses; this site lists the 9 Dies FSM UNDIP categories. Comparing
# their counts would report a permanent difference that says nothing about
# whether the animation hooks survived, so they are matched on presence and the
# count delta is reported separately.
DATA_DRIVEN_SIGNATURES = {
    "btn-plain sb__btn__link",
    "btn-plain sb__btn-apply",
}
...
def main() -> int:
    ...
    for route, (archived, built) in PAGES.items():
        ...
        # Fold data-driven repeats down to a presence check.
        notes = []
        for bag, other in ((missing, got), (extra, want)):
            for key in [k for k in bag if k[0] in DATA_DRIVEN_SIGNATURES]:
                if other[key]:
                    notes.append((key[0], want[key], got[key]))
                    del bag[key]
        ...
        for signature, before, after in sorted(notes):
            print(f"    catatan  .{signature}: {before} -> {after} (mengikuti data kategori)")
